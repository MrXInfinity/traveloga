...
const BookingUI = () => {
  ...
  return (
    <FormProvider {...form}>
      <div className="flex flex-row-reverse h-5/6 w-full md:w-3/4 lg:w-11/12 max-w-[1000px] bg-white">
        <FontAwesomeIcon className='z-10 text-4xl absolute p-6 lg:p-2 xl:p-6 ml-auto mb-auto lg:mr-1 text-black/50 transition-color duration-150 ease-in-out hover:text-black' icon={faXmark} onClick={() => closeBookingUI()}/>
        <form className="flex flex-col bg-white p-8 lg:py-6" onSubmit={handleSubmit(formSubmit)}>
          <h1 className='font-["Rubik"] text-2xl mb-2'>PLANE BOOKING</h1>
