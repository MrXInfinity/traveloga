      ...
      <div className="flex sm:touch-none lg:overflow-hidden overflow-x-scroll mt-4 ">
            <div className={`flex w-fit relative snap-x lg:snap-none touch-pan-x transition-all duration-500 delay-150 ease-in-out pb-2 lg:px-2`} style={{left: `${-leftPosition*35}rem`}} >
                {info.map(({title, image, location, _id})=>(
                  <div className="flex flex-col h-80 md:h-96 w-80 md:w-[400px] lg:w-[522px] shadow-lg hover:shadow-md hover:shadow-black rounded-[50px] md:rounded-none overflow-hidden mr-11 last:mr-6 snap-start transition-all duration-300 ease-in-out" key={_id} onClick={()=>destinationToggleUI(_id)}>
                    ...
                    <div className="flex h-fit pl-10 pr-4 py-3 md:px-4 md:py-4 justify-around lg:justify-between flex-col lg:flex-row">
                      <h1 className='text-lg md:text-xl lg:text-2xl my-auto'>{title}</h1>
                      <div className="flex text-black/70 lg:text-black/70 text-sm md:text-base lg:text-xl">
                        <FontAwesomeIcon icon={faLocationDot} className=' my-auto mr-1 lg:mr-2'/>
                        <h1 className='my-auto'>{location}</h1>
                      </div>
                    </div>    
                  </div>
                ))}
