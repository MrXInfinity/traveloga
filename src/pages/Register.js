  ...
  return (
    ...
    <div className='flex w-full h-screen bg-cover bg-no-repeat bg=center text-white font-["Spinnaker"] bg-[url("https://images.pexels.com/photos/4673637/pexels-photo-4673637.jpeg?cs=srgb&dl=pexels-rye-bon-4673637.jpg&fm=jpg")]'>
      <div className="flex flex-col md:w-full bg-gradient-to-r from-black/70 via-black/50 py-8 lg:py-6 px-6 lg:px-8">
        ...
        <div className="flex flex-col mx-4 lg:ml-12 mt-16 lg:mt-8 lg:w-7/12 h-full">
          <h1 className='font-["Rubik"] text-2xl mb-1'>CREATING A NEW ACCOUNT</h1>
          <p className='mb-6'>Already have an account? <span><Link to="/login" className='text-amber-200 hover:border-b-2 border-amber-200 inline h-fit transition-all duration-300 ease-in-out'>LOG IN</Link></span></p>
          <form className="flex flex-col lg:grid lg:grid-cols-2 gap-4 h-full" onSubmit={handleSubmit(submit)}>
            {formInputData.map(({title, inputName, icon, type, maxLength}, index)=>(
              <label className={`lg:text-sm ${index > 1 && `col-span-2`}`} key={index}>{title}<br />
                ...
                <div className="flex bg-white px-4 py-2 rounded-lg mt-2 lg:mt-3 items-center">
                  <input className='w-full bg-transparent text-lg lg:text-base py-1 text-black' type={!type ? typeIsPassword ? "password" : "text" : type} {...register(inputName, 
            ...
            <div className="grid grid-cols-2 lg:col-span-2 gap-4 lg:gap-12 mt-4 md:mt-auto">
              <Link className="flex p-4 bg-[#2B8E9B]/30 hover:bg-[#2B8E9B]/50 justify-center items-center transition-all ease-in-out" to="/">RETURN HOME</Link>
              <button className="flex bg-amber-200 hover:bg-amber-300 p-4 justify-center items-center transition-all ease-in-out">CREATE ACCOUNT</button>
            </div>
          </form>
        </div>
      </div>
