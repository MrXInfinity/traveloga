...
const Login = () => {
  ...
  return (
  <>
  <div className='flex w-full h-[93vh] md:h-screen bg-cover bg-no-repeat bg=center text-white font-["Spinnaker"] bg-[url("https://images.pexels.com/photos/4673637/pexels-photo-4673637.jpeg?cs=srgb&dl=pexels-rye-bon-4673637.jpg&fm=jpg")]'>
    <div className="flex flex-col lg:w-full bg-gradient-to-r from-black/70 via-black/50 py-8 lg:py-6 px-6 lg:px-8">
      ...
      <div className="flex flex-col mx-4 lg:ml-12 mt-10 md:mt-8 lg:w-7/12 md:h-full">
        <h1 className='font-["Rubik"] text-xl md:text-2xl mb-2 md:mb-3'>HELLO AGAIN!</h1>
        <p className='text-sm md:text-base'>Welcome valued customer! LOG IN to access your list of bookings and to also book future flights or edit existing ones.</p>
        {errors.login && <p className='text-red-600 my-4 text-xs md:stext-sm'>{errors.login.message}</p>}
        <form className={`flex flex-col gap-8 h-full ${errors.login ? `mt-0` : `mt-4`}`} onSubmit={handleSubmit(submit)}>
          ...
          <label className='md:text-lg ' key={index}>{title}<br />
      ...
      <h1 className="lg:w-7/12 lg:ml-12 pt-4 border-dashed border-t-2 md:text-lg text-center border-white mt-auto">
