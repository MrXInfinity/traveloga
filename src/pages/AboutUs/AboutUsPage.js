...
const AboutUsPage = () => {
  ...
  return (
    <>
      <div className='flex h-[93vh] lg:h-screen bg-cover bg-no-repeat bg-fixed md:pt-[6.5rem] lg:pt-[5.7rem]' style={{backgroundImage: `url("/images/aboutuspage-heropic.avif")`}}>
        <div className="flex flex-col items-center text-center md:items-start md:text-left h-fit md:h-full py-8 px-8 sm:py-16 md:py-10 lg:py-12 xl:py-14 place-self-end md:place-self-start md:w-7/12 lg:w-1/2  md:max-w-lg lg:max-w-xl md:justify-between lg:justify-start bg-[#423F3F]/90">
          <h1 className='font-["Rubik"] md:text-lg lg:text-xl text-amber-200 mb-4 sm:mb-2 lg:mb-6'>ABOUT US</h1>
          <p className='text-white text-2xl sm:text-4xl md:text-5xl lg:text-6xl mb-6 sm:mb-10 md:mb-6'>EXPERIENCE PHILIPPINES, <br className='md:block hidden'/>LOVE PHILIPPINES</p>
          <div className="bg-white h-1 w-full mb-4 md:mb-6 lg:mb-10"/>
          <p className='text-white text-sm sm:text-base md:text-lg lg:text-xl md:mb-4 lg:mb-0 md:text-justify'>Traveloga offers a safe, efficient and cheap way to book your favorites sites in the Philippines even if you’re coming from abroad or just locally. Here in Traveloga, we strive to make Philippines beloved by most and known by all</p>
        </div>
      </div>
      ...
      <div className="flex bg-white w-full justify-center max-w-7xl mx-auto">
        <div className="flex flex-col px-8 md:px-16 lg:px-10 py-8 lg:py-14 lg:w-5/6 xl:w-full w-full bg-[#423F3F] text-white text-center">
          <h1 className='text-xl md:text-2xl lg:text-3xl mb-4 font-["Rubik"]'>ORIGIN</h1>
          <p className='text-sm md:text-base lg:text-lg text-justify'>It all started as a group of friends that have a passion for traveling. We traveled all around the world, and we got to see a lot of beautiful things. When we decided to travel locally, we realized that the Philippines has untapped tourism potential. Through each expertise and overall hard work, we created Traveloga into what it is today. </p>
      </div>
      </div>
      ...
      <div className="flex bg-center bg-no-repeat bg-cover" style={{backgroundImage: `url("/images/progressivility.avif")`}}>
        <div className="flex flex-col w-full bg-[#423F3F]/80 text-white p-8 md:py-14 lg:py-16 md:px-16">
          ...
          <div className="flex flex-col sm:flex-row sm:gap-12 lg:gap-16 xl:gap-20 max-w-7xl">
